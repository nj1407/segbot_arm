...
public:
  ...
  TurnCorrectActionServer(std::string name) :
    as_(nh_, name, boost::bind(&TurnCorrectActionServer::executeCB, this, _1), false),
    action_name_(name)
  {

    
    //subscribe to odometry 
    sub_odom_= nh_.subscribe("/odom", 1,&TurnCorrectActionServer::odom_cb,this);
  ...
  ~TurnCorrectActionServer(void)
  {
    ...
    void executeCB(elevator_press_button::color_perception::Request &req, elevator_press_button::color_perception::Response &res){
    {
        if (goal->command == "approach"){
            ...
            ros::ServiceClient client_tabletop_perception = nh_.serviceClient<elevator_press_button::color_perception>("/pcl_button_filter/color_perception");
...
{
  ros::init(argc, argv, "turn_correct_as");

  TurnCorrectActionServer as(ros::this_node::getName());
  ros::spin();
